...
DIR_SAIDA       <- file.path(RAIZ, "TESTE/3_ANALISE_DE_GRUPOS/outputs_figuras/urbana_vs_rural")
...
metadados <- metadados |>
  rename(
    TIPO_ESCOLA = TIPO_ESCOLA_MODAL,
    AREA = ID_AREA_MODAL,
    LOCALIZACAO = ID_LOCALIZACAO_MODAL
  )
...
tabela <- dados_cluster |>
  ...
  select(ID_ESCOLA, LOCALIZACAO, AREA, TIPO_ESCOLA, CLUSTER,
