...
#
# VERSAO: 1.4 - Julho 2026 (v2: dicionario de perguntas/rotulos nos graficos
#   de coeficientes, offset Sig proporcional, altura 0.40, margem esquerda 90)
#
# ---------------------------------------------------------------------------
...
#    revelar os efeitos de maior magnitude. Cada barra e rotulada no eixo Y
#    por "CODIGO | rotulo da categoria" (ex.: TX_RESP_Q23d_C | Discordo),
#    usando dicionario extraido do INPUT_R do INEP e portado para
#    utils_saeb.r (DICT_ROTULOS_SAEB). As perguntas completas dos itens
#    presentes aparecem no caption da figura. Preditoras nao significativas
#    continuam no modelo (como controle) mas sao omitidas dos graficos por
#    legibilidade. IC 95% via quantil t (qt(0.975, gl)).
#
...
extrair_coef <- function(modelo) {
  ...
  tidy(modelo) |>
    mutate(
      ...
      # Rotulo legivel da categoria e pergunta (dicionarios de utils_saeb.r).
      # Para (Intercept) e dummies estruturais (TIPO_ESCOLA_/AREA_LOCAL_),
      # Item/Categoria ficam NA -> RotuloCat/Pergunta tambem ficam NA.
      RotuloCat = {
        # vectorizado: para cada linha, buscar no dicionario
        mapply(
          function(it, cat) {
            if (is.na(it) || is.na(cat)) return(NA_character_)
            rot <- DICT_ROTULOS_SAEB[[it]]
            if (is.null(rot)) return(NA_character_)
            if (cat %in% names(rot)) return(rot[[cat]])
            NA_character_
          },
          Item, Categoria,
          USE.NAMES = FALSE
      ...
      Pergunta = {
        mapply(
          function(it) {
            ...
            p <- DICT_PERGUNTAS_SAEB[[it]]
            if (is.null(p)) return(NA_character_) else p
          },
          Item,
          USE.NAMES = FALSE
        )
    ...
    select(
      Termo     = term,
      Item, Categoria, RotuloCat, Pergunta,
      Coef      = estimate,
      SE        = std.error,
      t_value   = statistic,
      p_valor   = p.value,
...
gerar_grafico_grupo <- function(coef_df, nome_modelo, cor_sig,
  ...
  dados_plot <- coef_df |>
    ...
    # Eixo Y: codigo | rotulo legivel da categoria (quebrado em ate 36 chars).
    # Mantem o codigo TX_RESP_Qnn_X para rastreabilidade com o CSV de coefs.
    mutate(
      RotuloEixo = if_else(
        !is.na(RotuloCat) & nchar(RotuloCat) > 0,
        paste0(Termo, " | ", str_wrap(RotuloCat, width = 36)),
        Termo
      ),
      Termo = fct_reorder(RotuloEixo, abs(Coef))
    )

  if (nrow(dados_plot) == 0) return(NULL)

  n_preds    <- nrow(dados_plot)
  # Altura por barra ampliada (0.40) para acomodar rótulos quebrados em 2 linhas.
  altura_fig <- max(5, n_preds * 0.40)

  #Lista de perguntas UNICAS dos itens presentes na figura (caption).
  perg_unicas <- dados_plot |>
    filter(!is.na(Pergunta)) |>
    distinct(Item, Pergunta) |>
    mutate(linha = paste0("  ", Item, ": ", str_wrap(Pergunta, width = 70))) |>
    pull(linha)
  bloco_perg <- if (length(perg_unicas) > 0) {
    paste0("\n\nPerguntas dos itens presentes:\n",
           paste(perg_unicas, collapse = "\n"))
  } else ""

  # Offset dos asteriscos de Sig proporcional aa escala do eixo X
  # (em vez de fixo +0.5/-0.5, que ficava sobreposto em coeficientes de
  # magnitude ~50 pontos SAEB). 4% do alcance conquista bom afastamento.
  ...
  p <- ggplot(dados_plot, aes(x = Termo, y = Coef)) +
    ...
    theme(
      ...
      # Margem esquerda ampliada (90) para acomodar rótulos longos quebrados.
      plot.margin   = margin(14, 20, 10, 90)
    )
