...
# (Baixamos o limiar para 0.20, pois a maior correlação para Matemática foi 0.22)
fortes_MT <- names(cor_MT[abs(cor_MT) >= 0.20])
dados_finais_MT <- dados_escalados[, fortes_MT, drop = FALSE] # <- drop=FALSE evita que vire vetor

print("--- VARIÁVEIS SELECIONADAS (MT >= 0.20) ---")
...
# (Baixamos o limiar para 0.20, pois a maior para Português foi 0.26)
fortes_LP <- names(cor_LP[abs(cor_LP) >= 0.20])
dados_finais_LP <- dados_escalados[, fortes_LP, drop = FALSE] # <- drop=FALSE evita que vire vetor

print("--- VARIÁVEIS SELECIONADAS (LP >= 0.20) ---")
...
df_fortes_MT <- df_correlacoes_MT[abs(df_correlacoes_MT$Correlacao_Matematica) >= 0.20, ]
...
df_fortes_LP <- df_correlacoes_LP[abs(df_correlacoes_LP$Correlacao_Portugues) >= 0.20, ]
